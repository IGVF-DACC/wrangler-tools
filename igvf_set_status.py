...
"""
This script is modified from the original version written for ENCODE:
https://github.com/ENCODE-DCC/pyencoded-tools/blob/master/ENCODE_set_status.py

For provided IGVF object identifiers, change (patch) stauts of each object,
as well as related objects, to the specific status. This is for DACC admin only.

Overall, this script accept one or more portal IDs through either
command line option `-r/--records` or a file and change their status to new
status specified by `-s/--status` option through IGVF "set_status" API. For
example:

    %(prog)s -r TSTDS22546470 -s released
    %(prog)s -i list_of_ids.txt -s archived

By default, the script will perform dry run via IGVF "set_status" API. To
make real change, use `--update` option:

    %(prog)s -r 6c8b0cec-324a-491f-b35f-c6bbbf7c7842 -s revoked --update

There are three other options which can be accepted by IGVF "set_status" API:
...
SERVER_MODES = {
    "dev": "https://igvfd-dev.demo.igvf.org",
...
class Connection():
    """Handles communication with the IGVF Portal regarding status change.
    ...
    def __init__(self, mode_host_key, keyfile=None, log_dir='.'):
        ...
        try:
            ...
            self.debug_logger.error(
                ...
                'Please check your key and host setting'.format(self.server_url)
            )
            raise
    ...
    def _setup_connection(self, key, keyfile=None):
        """
        The connection to IGVF portal is set up in the following precedence:

        1. Checkout `keyfile` if specified explicitly and get both URL and
        authentication from it based on the `key`.
        2. If a `keyfile` is not specified or setup connection from the
        provided `keyfile` failed, get authentication from environment
        variables, IGVF_API_KEY and IGVF_SECRET_KEY, and use `key` to get URL.
        ...
        both URL and authentication from it based on the `key`.
        """
        ...
        if keyfile:
            ...
            try:
                ...
                self.server_url = keypairs[key]['server']
                self.auth = (keypairs[key]['key'], keypairs[key]['secret'])
                self.debug_logger.info(
                    'Got connection setting from {}.'.format(path)
                )
                self.debug_logger.info('Host URL is: {}'.format(self.server_url))
                return
            except Exception:
        ...
        if os.getenv('IGVF_API_KEY', None) and os.getenv('IGVF_SECRET_KEY', None):
            self.auth = (os.getenv('IGVF_API_KEY'), os.getenv('IGVF_SECRET_KEY'))
            if key in SERVER_MODES:
                self.server_url = SERVER_MODES[key]
            elif str(key).startswith('http'):
                self.server_url = str(key)
            else:
                self.server_url = 'https://' + str(key).strip('/')
            self.debug_logger.info(
                'Got connection setting from environment variables: '
                'IGVF_API_KEY and IGVF_SECRET_KEY.'
            )
            self.debug_logger.info('Host URL is: {}'.format(self.server_url))
            return
    ...
    def touch_record(self, rec_id):
        try:
            url = '/'.join([self.server_url.rstrip('/'), rec_id.lstrip('/')])
    ...
    def set_status(self, object_id, status, update=False, force_audit=False,
        ...
        msg = (
            "<<<<<< SETTING {portal_id} status through URL {url} with this "
            "payload:\n\n{payload}\n\n"
        )
        ...
        self.debug_logger.debug("<<<<<< IGVF PATCH RESPONSE: ")
...
def get_parser():
    ...
    group.add_argument("-i", "--infile", help="""
                       An input file containing one or more record
    ...
    parser.add_argument("-m", "--mode-host-key", default='dev', help="""
                        ...
                        variables IGVF_API_KEY and IGVF_SECRET_KEY) or a key in
    ...
    parser.add_argument("--keyfile", help="""
                        ...
                        authentication and target portal.
    ...
    parser.add_argument("--block-children", action="store_true", help="""
                        Only change status for objects explicitly specified by
    ...
    parser.add_argument("--force-transition", action="store_true", help="""
                        Overide the transition table of IGVF API and force
